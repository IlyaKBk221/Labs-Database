...
-- Час останнього перезапуску сервера
...
exec sp_databases
-------БД
-- Перевірка від якої бд буде виконаний запит
select DB_NAME()
----Завдання Перелік та розшифровку значень type оформити у вигляді таблиці, 
----дивіться документацію на sys.objects у MSDN sys.objects.type
SELECT type AS 'Type',
       CASE type
       ...
       END AS 'Description'
FROM sys.objects;
-- Розташування файлів БД
-- фіз розташування можна дізнатися за доп 
exec sp_helpfile
-------Таблиці
--Метод повертає і таблиці, і представлення
SELECT @@Servername AS ServerName ,
DB_NAME() AS DBName ,
t.Name AS TableName,
t.[Type],
...
FROM sys.tables t
ORDER BY t.Name;
--Кількість записів у вибраній базі даних
...
-- Пошук кластерних індексів та куч (таблиць без кластерних індексів)
...
ORDER BY t.Name--Робота з кучами (+ кількість записів)SELECT @@ServerName AS Server ,
DB_NAME() AS DBName ,
...
--Більш просунута версія попереднього запиту
...
-- Отрим інформації про процедури, що зберіг та викон оп-ії INSERT
WHERE o.[type] = 'P' -- Stored procedures
AND sm.definition LIKE '%insert%'
order by o.name
...
--Перший метод - використовувати процедуру sp_msdependecies, що зберігається.
...
EXEC sp_msdependencies N'dbo.Dogovir',null, 1315327
EXEC sp_msdependencies N'dbo.Dogovir',null, 266751
EXEC sp_MSdependencies N'dbo.Dogovir', null, 1053183 
...
ORDER BY 1 ,
2;
--Третій метод - використовувати CTE (Сommon Table Expression).
...
SELECT 
    ...
    m.physical_name AS 'File Location'
FROM 
    sys.databases d 
JOIN 
    sys.master_files m 
